...
namespace DaggerfallWorkshop.Game.UserInterfaceWindows
{
    ...
    public class DaggerfallAutomapWindow : DaggerfallPopupWindow
    {
        ...
        private void UpdateButtonToolTipsText()
        {
            ...
            dummyPanelCompass.ToolTipText = String.Format("left click: toggle focus (hotkey: {0})\rred beacon: player, green beacon: entrance, blue beacon: rotation axis\r\rright click: reset view (hotkey: {1})\r\rdouble-click left mouse button in window to create a marker\rdouble-click left mouse button on a marker to add a note\rdouble-click right mouse button on a marker to delete it\rdouble-click right mouse button in window to position rotation axis\rdouble-click middle mouse button in window to center view", currentKeyCode_SwitchFocusToNextBeaconObject.ToString(), currentKeyCode_ResetView.ToString());
        }
        ...
        private void PanelAutomap_OnMouseDoubleClick(BaseScreenComponent sender, Vector2 position)
        {
            ...
            {
                automap.TryToAddOrEditUserNoteMarkerOnDungeonSegmentAtScreenPosition(mousePosition);
            }
        }
