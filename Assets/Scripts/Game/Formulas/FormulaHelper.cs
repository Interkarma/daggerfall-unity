...
namespace DaggerfallWorkshop.Game.Formulas
{
    ...
    public static class FormulaHelper
    {
        ...
        /// <param name="spellBundleTargetType">Source bundle target type for spell cost calculation.</param>
        /// <param name="caster">Source entity who cast the spell.</param>
        /// <param name="target">Target entity who got hit by the spell.</param>
        /// <param name="absorbSpellPointsOut">Number of spell points absorbed. Only valid when returning true.</param>
        /// <returns>Absorbed spell points. 0 if absorb was not successfull</returns>
        public static int CheckAndCalculateAbsorbedSpellPoints(IEntityEffect effect, TargetTypes spellBundleTargetType, DaggerfallEntityBehaviour caster, DaggerfallEntityBehaviour target, SpellAbsorption absorbIncumbentEffectOnTarget)
        {
            Func<IEntityEffect, TargetTypes, DaggerfallEntityBehaviour, DaggerfallEntityBehaviour, SpellAbsorption, int> del;
            if (TryGetOverride("CheckAndCalculateAbsorbedSpellPoints", out del))
                return del(effect, spellBundleTargetType, caster, target, absorbIncumbentEffectOnTarget);

            int absorbedSpellPoints = 0;

            // Nullchecks
            if (effect is null || target is null || caster is null)
                return 0;
            ...
            int effectCastingCost = GetEffectCastingCost(effect, spellBundleTargetType, target.Entity);

            // The target entity must have enough spell points free to absorb incoming effect
            int availableSpellPoints = target.Entity.MaxMagicka - target.Entity.CurrentMagicka;
            if (effectCastingCost > availableSpellPoints)
                return 0;
            else
                absorbedSpellPoints = effectCastingCost;

            // Handle effect-based absorption

            if (absorbIncumbentEffectOnTarget != null && TryEffectBasedAbsorption(effect, absorbIncumbentEffectOnTarget, target.Entity))
                return absorbedSpellPoints;

            // Handle career-based absorption
            if (target.Entity.Career.SpellAbsorption != DFCareer.SpellAbsorptionFlags.None && TryCareerBasedAbsorption(effect, target.Entity))
                return absorbedSpellPoints;

            // Handle persistant absorption (e.g. special advantage general/day/night or from weapon effects)
            if (target.Entity.IsAbsorbingSpells)
                return absorbedSpellPoints;
